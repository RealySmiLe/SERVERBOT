...
client.on('message',async message => {
  ...
  message.guild.createChannel(`.Miracle Online: [ ${message.guild.members.filter(m => m.voiceChannel).size} ]` , 'voice').then(c => {
    ...
    setInterval(() => {
      c.setName(`.Miracle Online :[${message.guild.members.filter(m => m.voiceChannel).size}]`)
    },1000);
  });
  }
});
...
.addField(` :WELOCOME TO SERVER Miracle
...
client.on('message', message => {
  ...
  if (message.content === 'ولكم') {
      message.channel.send(`*WELCOME TO SERVER MiRCALE :heart: :yellow_heart:*`)
  }
